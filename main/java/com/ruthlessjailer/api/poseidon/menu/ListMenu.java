package com.ruthlessjailer.api.poseidon.menu;

import com.google.common.primitives.Ints;
import com.ruthlessjailer.api.poseidon.item.ItemBuilder;
import com.ruthlessjailer.api.poseidon.menu.button.ActionButton;
import com.ruthlessjailer.api.poseidon.menu.button.ButtonAction;
import com.ruthlessjailer.api.poseidon.menu.button.ButtonBase;
...
public abstract class ListMenu<I extends ListItem> extends MenuBase {
	...
	public ListMenu(final MenuBase parent, final int size, final @NonNull String title) {
		...
		final ItemBuilder button = ItemBuilder.from(Material.ARROW).flags(ItemFlag.values());

		setBackButton(button.displayName("&6&m&l<-&c&a " + DESTINATION_PAGE_PLACEHOLDER + "&8/&9" + TOTAL_PAGES_PLACEHOLDER).build());
		setNextButton(button.displayName("&a" + DESTINATION_PAGE_PLACEHOLDER + "&8/&9" + TOTAL_PAGES_PLACEHOLDER + " &6&m&l->").build());
	}
	...
	public CompletableFuture<MenuBase> updateInventory() {
		return FutureHandler.Companion.getAsync().supply(() -> {
			generateInventory();
	...
	protected CompletableFuture<MenuBase> generateInventory() {
		if (!this.pages.isEmpty()) {
			return FutureHandler.Companion.getAsync().supply(() -> this);
		}
...
//			} catch (final NumberFormatException ignored) {}//localized name can be null; doesn't have to have page number
			...
			button.setItem(builder.displayName(formatName(button.getItem(), this.pages.size())).build());//this is called during page generation
		});
	}
	...
	private void setButtonInMenu(final int destinationPageNumber, final int slot, @NonNull final ButtonBase button, @NonNull final Map<Integer, ButtonBase> buttons) {
		...
		modified.setItem(ItemBuilder.from(button.getItem())
									.displayName(name)
									.localizedName(String.valueOf(destinationPageNumber)).build());
