...
public abstract class MenuBase {
	...
	public MenuBase(final MenuBase parent, final int size, @NonNull final String title) {
		...
		this.parent = parent;
		this.size   = size;
		this.title  = Chat.INSTANCE.colorize(title);
		this.type   = null;

		setPreviousMenuButton(ItemBuilder.from(Material.ARROW).displayName("&9&l&m<-&c&3 Return").flags(ItemFlag.values()).build());
	}
	...
	private static MenuBase getMenu(@NonNull final Player player, @NonNull final String tag) {
		...
		if (meta.isEmpty()) {
			Chat.INSTANCE.warning("Player " + player.getName() + "'s metadata " + tag + " is corrupted; cannot retrieve the menu.");
			return null;
		}
	...
	public void displayTo(@NonNull final Player player) {
		...
		if (current != null) {
			TaskManager.Companion.getSync().later(() -> player.setMetadata(NBT_PREVIOUS_MENU, new FixedMetadataValue(Checks.instanceCheck(), current)));
		}

		TaskManager.Companion.getSync().later(() -> player.setMetadata(NBT_CURRENT_MENU, new FixedMetadataValue(Checks.instanceCheck(), this)));
