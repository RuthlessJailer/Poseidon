...
abstract class CommandBase : Command {
	...
	companion object Info {
		...
		const val DEFAULT_PERMISSION_MESSAGE = "&cYou do not have the permission &3\${permission}&c needed to run this com.ruthlesjailer.api.poseidon.command!"
		const val DEFAULT_PERMISSION_SYNTAX = "\${plugin.name}.com.ruthlesjailer.api.poseidon.command.\${com.ruthlesjailer.api.poseidon.command.label}"
		const val DEFAULT_SUB_COMMAND_PERMISSION_SYNTAX = "\${permission}.\${sub.com.ruthlesjailer.api.poseidon.command}"
		const val DEFAULT_PLAYER_FALSE_MESSAGE = "&cThis com.ruthlesjailer.api.poseidon.command must be executed by a player!"

		var starPermissionSyntax = PluginBase.instance?.name?.let {
			DEFAULT_PERMISSION_SYNTAX
					.replace("\${plugin.name}", it)
					.replace("\${com.ruthlesjailer.api.poseidon.command.label}", "*")
		}.toString()
			protected set
	...
	var customPermissionSyntax: String = DEFAULT_PERMISSION_SYNTAX
			...
			.replace("\${com.ruthlesjailer.api.poseidon.command.label}", label)
		protected set(value) {
			field = value
					.replace("\${plugin.name}", PluginBase.getCurrentName().toLowerCase())
					.replace("\${com.ruthlesjailer.api.poseidon.command.label}", label)
	...
	final override fun execute(sender: CommandSender, commandLabel: String, args: Array<String>): Boolean {
		...
		try {
			...
			if (!(autoGenerateHelpMenu && args.isNotEmpty() && args[0].equals("help", ignoreCase = true))) { //don't run on help com.ruthlesjailer.api.poseidon.command
...
//			SubCommandManager.generateHelpMenu(this, helpMenuFormatOverride)
	...
	/**
	 * Checks if given [Permissible] [is op][Permissible.isOp], has the [star permission][CommandBase.getStarPermissionSyntax], has the
	 * [com.ruthlesjailer.api.poseidon.command&#39;s permission][CommandBase.getCustomPermissionSyntax], or has the given permission.
	 *
	 * @param permissible the [Permissible] to check
	 * @param permission  the permission to check (last resort)
	 *
	 * @return `true` if the [Permissible] [is op][Permissible.isOp], has the [star permission][CommandBase.getStarPermissionSyntax],
	 * has the [com.ruthlesjailer.api.poseidon.command&#39;s permission][CommandBase.getCustomPermissionSyntax], or has the given permission; `false` if either argument is null or the
